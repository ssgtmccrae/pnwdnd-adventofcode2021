...
def find_fuel_cost(point_list, test_num):
    ...
    Computing for Pt2 of Day07 Challenge.
    Input: point_list (List[int]), test_num (int)
    Output: int, total fuel cost
    """
    total_cost = 0
    for point in point_list:
        num_moves = abs(point - test_num)
        total_cost += (num_moves * (num_moves + 1)) / 2
    return total_cost
...
def find_meeting_point(point_list):
    ...
    Output: None
    """
    list_avg = int(round(sum(point_list) / len(point_list)))
...
if __name__ == '__main__':
    ...
    # dataset = [16,1,2,0,4,2,7,1,2,14] # Uses 168 Fuel w/ an meeting point of 5
