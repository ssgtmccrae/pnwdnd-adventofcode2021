...
func duplicateNumber(sn *SnailNum) *SnailNum {
	if sn.IsPair {
		return &SnailNum{
			IsPair: true,
			Left:   duplicateNumber(sn.Left),
			Right:  duplicateNumber(sn.Right),
		}
	} else {
		return &SnailNum{IsPair: false, Value: sn.Value}
	}
}
...
func runData(data string) {
	...
	possibleMagnitues := []int{}
	for _, snl := range nums {
		for _, snr := range nums {
			possibleMagnitues = append(
				possibleMagnitues,
				getMagnitude(reduceNumber(&SnailNum{
					IsPair: true,
					Left:   duplicateNumber(snl),
					Right:  duplicateNumber(snr),
				})),
			)
		}
	}
	...
	largest := 0
	for _, m := range possibleMagnitues {
		if m > largest {
			largest = m
		}
	}
	fmt.Println("Maginutes: ", possibleMagnitues)
